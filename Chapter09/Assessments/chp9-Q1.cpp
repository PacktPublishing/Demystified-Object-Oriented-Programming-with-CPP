...
// Sample solution to Chapter 9, Question 1, parts a, b, c
// a. There is one LifeForm sub-object.
// b. The LifeForm constructor and destructor are each invoked once.
// c. The default constructor for LifeForm would be invoked if the specifications of an alternate constructor
//    of :LifeForm(CENTAUR_LIFE) was removed in the member initialization list of the Centaur constructors.

// Purpose: To illustrate virtual base classes.
// Virtual base classes resolve duplications which can exist with multiple inheritance when two or more 
// base classes for a class share a common ancestor class.  
// In this example, only one copy of the LifeForm sub-object will exist for each instance of a Centaur.
...
public:
   Centaur(): LifeForm(CENTAUR_LIFE) { cout << "Centaur constructor" << endl; }
